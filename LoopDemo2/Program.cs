...
namespace LoopDemo2
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while (keepGoing) // The reason you don't need to say keepGoing == true is because keepGoing is already declared with that value above
            {
            ...
            for (int j = 0; j <= 20; j++)
            {
                ...
                if (j %2 == 0)
                {
                    Console.WriteLine($"j is even. j times itself is {j * 2}"); // This loop is the same as above, but it is "cleaner" code because the continue above just breaks out of the loop and CW's this anyway
                }
            }
        }
    }
